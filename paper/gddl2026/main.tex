...
family. An exact modal decomposition attributes the mismatch to solver-stage
...
Euler. This is a controlled limitation study: it does not
refute regularity-guided schedule design, and no learned model is
evaluated.
\end{abstract}
...
$1.1189\times10^{-5}$, exceeds this 11{,}500-fold (artifact
\artifact{A2}).
...
\begin{figure}[t]
  ...
  $\wtwo$-preferred path, always opposite to the baseline (artifact
  \artifact{A1}).}
...
established~\citep{sabour2024align}; the point is that no
solver-independent scalar can represent it.
...
artifact \artifact{A5}). The family remains Gaussian, diagonal, and
...
is evaluated, no image, video, or world-model improvement is demonstrated
or implied, and the proposition is grid-aware. Exponential and
exact-linear integrators solve affine fields exactly: the result concerns
generic fixed-stage Runge--Kutta discretization, not best-possible
solvers; learned fields are generally non-affine, but no learned-field
result is established here. We rank two fixed paths at the
identity time parameterization; transfer-based schedule
optimization~\citep{lipschitzguided2025} is complementary and not
addressed.
Averaged regularity may remain valuable for schedule design, and our
result does not refute Lipschitz-guided schedules: in the tested systems the
averaged-regularity \emph{ordering} does not determine the fixed-NFE
$\wtwo$ ordering,
because fixed-budget error lives in solver-stage samples and transported
signed defects the average cannot see. All numerical statements resolve
to checksummed audit artifacts (A1--A5) in the supplement.
